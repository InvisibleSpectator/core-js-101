...
function getPolynom(...coeff) {
  ...
  const rev = coeff.reverse();
  return function b(x) {
    const ret = rev.reduce((a, e, i) => a + (x ** i) * e, 0);
    return ret;
  };
}
...
function logger(func, logFunc) {
  return function b(...a) {
    ...
    a.forEach((e) => {
      if (typeof e === 'object') {
        ret += `[${e.map((w) => { if (typeof w === 'string') return `"${w}"`; return w; })}],`;
      } else {
        ret += `${e},`;
      }
    });
    logFunc(`${ret.slice(0, ret.length - 1)}) starts`);
    const z = func(...a);
    logFunc(`${ret.slice(0, ret.length - 1)}) ends`);
    return z;
  };
}
